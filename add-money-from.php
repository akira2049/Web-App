...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'new_card') {
    ...
    $no   = preg_replace('/\s+/', '', $_POST['card_no'] ?? '');
    ...
    if ($name === '' || $no === '' || $exp === '' || $cvv === '') {
        ...
        $_SESSION['am_from']        = "VISA/Mastercard";
...
<head>
  ...
  <style>
    ...
    .saved-card-label{font-weight:600;}
  </style>
</head>
<body>
  <div class="app">
    ...
    <div class="card">
      <div class="section">
        <div class="tabs" role="tablist">
          <button id="t1" class="tab" role="tab" aria-selected="true">Saved Card</button>
          <button id="t2" class="tab" role="tab" aria-selected="false">New Card</button>
        </div>

        <!-- SAVED CARDS PANEL -->
        <div id="panel-saved" role="tabpanel">
            ...
            <div class="saved-list">
              ...
              <form method="POST">
                ...
                <button class="saved-card-item">
                  <span class="saved-card-label">Saved Card</span>
        ...
        <!-- NEW CARD FORM -->
        <form id="cardForm" method="POST" role="tabpanel" hidden>
          <input type="hidden" name="action" value="new_card">

          <div class="row">
            <div class="label">Cardholder Name</div>
            <input class="input" name="card_name" required placeholder="Enter cardholder name">
          </div>

          <div class="row">
            <div class="label">Card Number</div>
            <input class="input" name="card_no" required inputmode="numeric" minlength="13" maxlength="19" placeholder="Enter card number">
          </div>

          <div class="row">
            <div class="label">Expiry Date</div>
            <input type="month" class="input" name="card_exp" required placeholder="MM/YY">
          </div>

          <div class="row">
            <div class="label">CVV</div>
            <input class="input" name="card_cvv" required inputmode="numeric" minlength="3" maxlength="4" placeholder="CVV">
          </div>

          <div class="row">
            <div class="label"></div>
            <label class="kv"><input type="checkbox" name="saveCard" checked> Save this card information</label>
          </div>
